...
function folded_menu_default() {
    if ($("body").hasClass("sidebar-left-hidden")) {
        ...
        $("#menu").find("li").has("ul").children("a").on("click", function (e) {
            e.preventDefault();
            $(this).parent("li").addClass("active").children("ul").collapse("toggle");
            $(this).parent("li").siblings().removeClass("active").children("ul.in").collapse("hide");
        });
    }
}
$("body").on('left_change', function() {
    setTimeout(function () {
        folded_menu_default();
    },300);
});
...
//        Full screen
    ...
    var folded=$('#setting_folded_menu').find(input);
    ...
    if($(window).width()>767) {
        ...
        folded.on('lcs-statuschange', function () {
            if ($("#setting_folded_menu").find(".lcs_switch").hasClass(".lcs_on")) {
        ...
        folded.on('lcs-statuschange', function () {
            $("body").toggleClass("sidebar-left-hidden").trigger("left_change");
        });
    }
