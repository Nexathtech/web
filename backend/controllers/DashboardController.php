...
final class DashboardController extends BaseController
{
    ...
    {
        $daysLimit = 30;
        $prints = Action::find()->where(['type' => Type::PRINT])->all();
        $lastDaysPrints = Action::find()->select('DATE(created_at) created_at, COUNT(id) id, data')->where(['type' => Type::PRINT])->orderBy('created_at DESC')->groupBy('DATE(created_at), id')->limit($daysLimit)->all();
